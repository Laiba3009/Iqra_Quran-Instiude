...
export default function SendNoticePage() {
  ...
  // 🔹 Fetch all active (not deleted) notices
  ...
  const sendNotice = async () => {
    if (!title.trim() || !message.trim()) {
      return toast({ title: 'Missing Fields', description: 'Please enter both title and message.' });
    }
    ...
    toast({ title: 'Notice Sent ✅', description: 'Your notice has been posted successfully.' });
    setTitle('');
    setMessage('');
    setVisibleTo('all');
    loadNotices();
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <Card className="shadow-lg border">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-green-700">Send New Notice</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <Button onClick={sendNotice} className="bg-green-600 hover:bg-green-700 text-white">
            🚀 Send Notice
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="shadow-lg border">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-green-700">Existing Notices</CardTitle>
        </CardHeader>
        <CardContent>
          {notices.length === 0 ? (
            ...
            <ul className="space-y-4">
              {notices.map((n) => (
                ...
                >
                  <div className="w-[85%]">
                    <h3 className="font-semibold text-lg text-green-800">{n.title}</h3>
